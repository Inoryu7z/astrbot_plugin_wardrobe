(function(){
  ...
  async function loadImages(resetGrid){
    ...
    if(resetGrid){
      ...
      $('#imageGrid').innerHTML='';
      if(state.sort_by==='random'){
        state._randomPool=[];
        state._randomPoolIdx=0;
      }
    }
    ...
    if(state.sort_by==='random' && !resetGrid && state._randomPool && state._randomPoolIdx<state._randomPool.length){
      const end=Math.min(state._randomPoolIdx+state.perPage,state._randomPool.length);
      images=state._randomPool.slice(state._randomPoolIdx,end);
      state._randomPoolIdx=end;
      total=state.total;
    }else if(!resetGrid && !state.searchQuery && state.preloadedPage2){
      ...
      const data=await resp.json();
      if(state.sort_by==='random' && !state.searchQuery){
        state._randomPool=data.images||[];
        state._randomPoolIdx=Math.min(state.perPage,state._randomPool.length);
        images=state._randomPool.slice(0,state._randomPoolIdx);
        total=data.total||state._randomPool.length;
      }else{
        images=data.images||[];
    ...
    if(!state.searchQuery && state.sort_by!=='random' && images.length<state.perPage){
      state.allLoaded=true;
    }else if(!state.searchQuery && state.sort_by==='random'){
      if(state._randomPool && state._randomPoolIdx>=state._randomPool.length){
        state.allLoaded=true;
      }
    ...
    if(resetGrid && !state.searchQuery && state.sort_by!=='random'){
  ...
  async function appendGrid(images){
    ...
    cardsWithHeights.forEach(({img,height,aspectRatio})=>{
      ...
      card.addEventListener('mousedown',e=>{
        ...
        state.dragSelectAction=state.selectedIds.has(img.id)?'deselect':'select';
        if(state.dragSelectAction==='select')state.selectedIds.add(img.id);
        else state.selectedIds.delete(img.id);
        updateBatchUI();
      });
  ...
  // ---------- 池子管理（统一状态） ----------
  let _poolsState={currentTab:'global',globalStylePool:[],allPools:{},personas:[],personaPools:{},searchFilter:''};

  async function loadPoolsModal(){
    const [poolsResp,personaResp]=await Promise.all([api('/api/pools'),api('/api/persona-style-pools')]);
    if(!poolsResp)return;
    const poolsData=await poolsResp.json();
    const pools=poolsData.pools||{};
    _poolsState.allPools=pools;
    _poolsState.globalStylePool=pools.style||[];
    if(personaResp){
      try{
        const pData=await personaResp.json();
        _poolsState.personas=pData.personas||[];
        _poolsState.personaPools=pData.pools||{};
      }catch(e){console.error('loadPersonaPools',e);}
    }
    renderStyleTabs();
    renderStylePool(_poolsState.currentTab);
    renderTagsList(pools);
  }

  function renderStyleTabs(){
    const tabContainer=$('#poolsStyleTabs');
    const tabs=[{target:'global',label:'全局'}];
    _poolsState.personas.forEach(p=>{
      const count=(_poolsState.personaPools[p]||[]).length;
      tabs.push({target:p,label:p+(count>0?` (${count})`:'')});
    });
    tabContainer.innerHTML=tabs.map(t=>`<button class="pools-style-tab${t.target===_poolsState.currentTab?' active':''}" data-target="${esc(t.target)}">${esc(t.label)}</button>`).join('');
    tabContainer.querySelectorAll('.pools-style-tab').forEach(btn=>{
      btn.addEventListener('click',()=>{
        _poolsState.currentTab=btn.dataset.target;
        _poolsState.searchFilter='';
        const si=$('#styleSearchInput');if(si)si.value='';
        renderStyleTabs();
        renderStylePool(_poolsState.currentTab);
      });
    });
  ...
  async function renderStylePool(tab){
    const list=$('#stylePoolList');
    if(!list)return;
    if(tab==='global'){
      // 全局风格池：增删式 tag
      let styles=[..._poolsState.globalStylePool];
      const f=_poolsState.searchFilter;
      if(f)styles=styles.filter(s=>s.toLowerCase().includes(f.toLowerCase()));
      if(styles.length===0){
        list.innerHTML='<span class="pools-hint" style="opacity:0.5;">'+(f?'无匹配风格':'全局风格池为空')+'</span>';
        return;
      }
      ...
      list.querySelectorAll('.pool-tag-remove').forEach(btn=>{
        btn.addEventListener('click',async()=>{
          ...
          const resp=await api('/api/pools',{method:'POST',json:{key:'style',action:'remove_value',value}});
          if(resp&&resp.ok){
            _poolsState.globalStylePool=_poolsState.globalStylePool.filter(s=>s!==value);
            toast('已移除「'+value+'」','success');
            renderStyleTabs();renderStylePool('global');renderTagsList(_poolsState.allPools);loadFilters();
          }
        });
      });
    }else{
      // 人格风格池：勾选式
      const persona=tab;
      const globalStyles=_poolsState.globalStylePool;
      const personaStyles=_poolsState.personaPools[persona]||[];
      const personaSet=new Set(personaStyles);
      let styles=globalStyles;
      // 人格池里有但全局池没有的，也要显示
      const extra=personaStyles.filter(s=>!globalStyles.includes(s));
      if(extra.length>0)styles=[...globalStyles,...extra];
      const f=_poolsState.searchFilter;
      let filtered=styles;
      if(f)filtered=styles.filter(s=>s.toLowerCase().includes(f.toLowerCase()));
      if(filtered.length===0){
        list.innerHTML='<span class="pools-hint" style="opacity:0.5;">'+(f?'无匹配风格':'无可用风格')+'</span>';
        return;
      }
      list.innerHTML='<div class="pools-hint" style="margin-bottom:0.5rem;font-size:0.8rem;opacity:0.6;">勾选 = 该人格使用此风格；全部不勾选 = 回退到全局池。当前已选 '+personaStyles.length+' 项</div>'+
        filtered.map(v=>`<label class="style-checkbox-tag${personaSet.has(v)?' checked':''}"><input type="checkbox" value="${esc(v)}" ${personaSet.has(v)?'checked':''}><span>${esc(v)}</span></label>`).join('');
      list.querySelectorAll('input[type=checkbox]').forEach(cb=>{
        cb.addEventListener('change',async()=>{
          const val=cb.value;
          const checked=cb.checked;
          cb.closest('.style-checkbox-tag').classList.toggle('checked',checked);
          let newStyles;
          if(checked){
            newStyles=[...personaStyles,val];
            _poolsState.personaPools[persona]=newStyles;
          }else{
            newStyles=personaStyles.filter(s=>s!==val);
            _poolsState.personaPools[persona]=newStyles;
          }
          const resp=await api('/api/persona-style-pools',{method:'POST',json:{persona:persona,styles:newStyles}});
          if(resp&&resp.ok){
            toast(checked?'已添加到人格池':'已从人格池移除','success');
            renderStyleTabs();
            renderStylePool(persona);
          }else{
            // 回滚
            _poolsState.personaPools[persona]=personaStyles;
            cb.checked=!checked;
            cb.closest('.style-checkbox-tag').classList.toggle('checked',!checked);
            toast('操作失败','error');
          }
  ...
  function renderTagsList(pools){
    const list=$('#poolsList');
    if(!list)return;
    list.innerHTML='';
    // 排除 style（已在风格池管理中展示）
    const entries=Object.entries(pools).filter(([k])=>k!=='style').sort((a,b)=>(poolLabel(a[0])).localeCompare(poolLabel(b[0])));
  ...
  function init(){
    ...
    $('#styleSearchInput').addEventListener('input',e=>{
      _poolsState.searchFilter=e.target.value;
      renderStylePool(_poolsState.currentTab);
    });
    $('#styleAddBtn').addEventListener('click',async()=>{
      const input=$('#styleNewInput');
      const val=input.value.trim();
      if(!val){toast('请输入风格名','error');return;}
      if(_poolsState.currentTab==='global'){
        if(_poolsState.globalStylePool.includes(val)){toast('已存在','info');return;}
        const resp=await api('/api/pools',{method:'POST',json:{key:'style',action:'add_value',value:val}});
        if(resp&&resp.ok){
          _poolsState.globalStylePool=[..._poolsState.globalStylePool,val];
          toast('已添加','success');input.value='';
          renderStyleTabs();renderStylePool('global');loadFilters();
        }
      }else{
        const persona=_poolsState.currentTab;
        const styles=_poolsState.personaPools[persona]||[];
        if(styles.includes(val)){toast('已存在','info');return;}
        const newStyles=[...styles,val];
        const resp=await api('/api/persona-style-pools',{method:'POST',json:{persona:persona,styles:newStyles}});
        if(resp&&resp.ok){
          _poolsState.personaPools[persona]=newStyles;
          toast('已添加','success');input.value='';
          renderStyleTabs();renderStylePool(persona);
        }
      }
    });
    $('#styleNewInput').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();$('#styleAddBtn').click();}});
    $('#styleResetBtn').addEventListener('click',async()=>{
      if(!confirm('确定恢复默认风格池？所有自定义增删将被清除。'))return;
      const resp=await api('/api/pools',{method:'POST',json:{key:'style',action:'reset_default'}});
      if(resp&&resp.ok){
        const data=await resp.json();
        _poolsState.globalStylePool=data.style||[];
        toast('已恢复默认','success');
        renderStyleTabs();renderStylePool('global');loadFilters();
      }
    });
    ...
    $('#videoViewBtn').addEventListener('click',()=>{
    ...
    $('#vgpCloseBtn').addEventListener('click',hideVideoGeneratePanel);

    $$('#videoGeneratePanel .vgp-tier-btn').forEach(btn=>{
      btn.addEventListener('click',()=>{
    ...
    $('#videoUmoSaveBtn').addEventListener('click',saveVideoUmoSettings);
    ...
    $('#vpSendBtn').addEventListener('click',()=>{
      if(state.videoCurrentPlayerId)sendVideoToConversation(state.videoCurrentPlayerId);
    });
  ...
  function _tierLabel(tier){
    ...
    if(tier==='light_spicy')return '轻荤';
    if(tier==='heavy_spicy')return '重荤';
    return tier||'';
  }

  function _tierClass(tier){
    if(tier==='normal')return 'vgp-tier-normal';
    if(tier==='light_spicy')return 'vgp-tier-light';
    if(tier==='heavy_spicy')return 'vgp-tier-heavy';
    return '';
  }
  ...
  function _statusClass(status){
    if(status==='generating')return 'video-status-generating';
    if(status==='done')return 'video-status-done';
    if(status==='failed')return 'video-status-failed';
    return '';
  }
  ...
  function _escapeAttr(s){return s?String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;'):'';}
  ...
  function updateVideoPagination(){
    const pag=$('#videoPagination');
    pag.innerHTML='';
    if(state.videoHasMore){
      const btn=document.createElement('button');
      btn.className='btn btn-secondary btn-sm';
      btn.textContent='加载更多';
      btn.addEventListener('click',()=>loadVideos(false));
      pag.appendChild(btn);
    }
  }

  function startVideoPoll(videoId){
    if(state.videoPollIntervals[videoId])return;
    state.videoPollIntervals[videoId]=setInterval(()=>checkVideoStatus(videoId),10000);
  }

  function stopVideoPoll(videoId){
    if(state.videoPollIntervals[videoId]){
      clearInterval(state.videoPollIntervals[videoId]);
      delete state.videoPollIntervals[videoId];
    }
  }

  async function checkVideoStatus(videoId){
    try{
      const resp=await api('/api/videos/'+videoId);
      if(!resp||!resp.ok)return;
      const data=await resp.json();
  ...
  function showVideoGeneratePanel(){
    const panel=$('#videoGeneratePanel');
    panel.classList.remove('hidden');
    $('#vgpThoughts').value='';
    $('#vgpBackend').value='';
    $('#vgpStatus').classList.add('hidden');
    $('#vgpSubmitBtn').disabled=false;
    $('#vgpSubmitBtn').textContent='开始生成';
    state.videoGenerating=false;
  }

  function hideVideoGeneratePanel(){
    $('#videoGeneratePanel').classList.add('hidden');
  }

  async function generateVideo(tier){
    if(!state.currentImageId){toast('没有选中图片','error');return;}
    if(state.videoGenerating)return;
    state.videoGenerating=true;
    const statusEl=$('#vgpStatus');
    const submitBtn=$('#vgpSubmitBtn');
    submitBtn.disabled=true;
    submitBtn.textContent='提交中...';
    statusEl.classList.remove('hidden');
    statusEl.textContent='正在提交视频生成任务...';
    statusEl.style.color='var(--text-muted)';
    const thoughts=$('#vgpThoughts').value.trim();
    const backend=$('#vgpBackend').value.trim();
    const autoSend=$('#vgpAutoSend').checked;
    try{
      const resp=await api('/api/videos/generate',{
        method:'POST',
        json:{image_id:state.currentImageId,tier:tier,user_thoughts:thoughts||null,backend_override:backend||null,auto_send:autoSend}
      });
      if(!resp||!resp.ok){
  ...
  function openVideoPlayer(videoId){
    state.videoCurrentPlayerId=videoId;
    api('/api/videos/'+videoId).then(r=>{
      if(!r||!r.ok)return;
      return r.json();
    }).then(data=>{
      if(!data||!data.video)return;
      ...
      $('#vpMetaTime').textContent=_fmtRelative(v.created_at);
      if(v.generated_prompt){$('#vpPromptBlock').classList.remove('hidden');$('#vpPromptText').textContent=v.generated_prompt;}
      else{$('#vpPromptBlock').classList.add('hidden');}
      ...
      const vpSendBtn=$('#vpSendBtn');
      if(v.status==='done'){vpSendBtn.classList.remove('hidden');}else{vpSendBtn.classList.add('hidden');}
  ...
  function closeVideoPlayer(){
    ...
    state.videoCurrentPlayerId=null;
  }

  async function deleteVideo(videoId,fromPlayer){
    if(!confirm('确定删除此视频？此操作不可恢复。'))return;
    try{
      const resp=await api('/api/videos/'+videoId,{method:'DELETE'});
      if(!resp||!resp.ok){toast('删除失败','error');return;}
      const data=await resp.json();
      if(data.success){
        toast('视频已删除','success');
        if(fromPlayer){closeVideoPlayer();}
        if(state.videoViewActive){
          const card=$('#videoGrid').querySelector('[data-video-id="'+videoId+'"]');
          if(card)card.remove();
          if($('#videoGrid').children.length===0){state.videoPage=1;state.videoHasMore=true;loadVideos(true);}
        }
  ...
  async function loadVideoSettings(){
    try{
      const resp=await api('/api/video-settings/prompt');
      if(!resp||!resp.ok)return;
      const data=await resp.json();
      $('#videoSystemPrompt').value=data.prompt||'';
    }catch(e){$('#videoSystemPrompt').value='加载失败: '+e.message;}
    try{
      const resp=await api('/api/video-settings/umo');
      if(!resp||!resp.ok)return;
      const data=await resp.json();
      $('#videoSendUmo').value=data.umo||'';
      $('#videoAutoSend').checked=!!data.auto_send;
    }catch(e){}
  }

  async function saveVideoSettings(){
    const text=$('#videoSystemPrompt').value;
    const statusEl=$('#videoPromptSaveStatus');
    statusEl.textContent='保存中...';
    statusEl.style.color='var(--text-muted)';
    try{
      const resp=await api('/api/video-settings/prompt',{method:'POST',json:{prompt:text}});
      if(!resp||!resp.ok){statusEl.textContent='保存失败';statusEl.style.color='var(--danger)';return;}
      const data=await resp.json();
      if(data.success){statusEl.textContent='已保存';statusEl.style.color='#40A060';setTimeout(()=>{statusEl.textContent='';},3000);}
      else{statusEl.textContent=data.error||'保存失败';statusEl.style.color='var(--danger)';}
    }catch(e){statusEl.textContent='网络错误: '+e.message;statusEl.style.color='var(--danger)';}
  }

  async function saveVideoUmoSettings(){
    const umo=$('#videoSendUmo').value.trim();
    const autoSend=$('#videoAutoSend').checked;
    const statusEl=$('#videoUmoSaveStatus');
    statusEl.textContent='保存中...';
    statusEl.style.color='var(--text-muted)';
    try{
      const resp=await api('/api/video-settings/umo',{method:'POST',json:{umo:umo,auto_send:autoSend}});
      if(!resp||!resp.ok){statusEl.textContent='保存失败';statusEl.style.color='var(--danger)';return;}
      const data=await resp.json();
      if(data.success){statusEl.textContent='已保存';statusEl.style.color='#40A060';setTimeout(()=>{statusEl.textContent='';},3000);}
      else{statusEl.textContent=data.error||'保存失败';statusEl.style.color='var(--danger)';}
  ...
  async function sendVideoToConversation(videoId){
    try{
      const resp=await api('/api/videos/'+videoId+'/send',{method:'POST'});
      if(!resp||!resp.ok){
  ...
  async function loadVideoPersonaFilter(){
    state._videoFiltersLoaded=true;
    try{
      const resp=await api('/api/filters');
      if(!resp||!resp.ok)return;
  ...
  async function loadImageVideos(imageId){
    try{
      const resp=await api('/api/videos?source_image_id='+imageId);
      if(!resp||!resp.ok)return;
      const data=await resp.json();
      const videos=data.videos||[];
      const section=$('#videoListSection');
      const grid=$('#videoListGrid');
      if(videos.length===0){section.classList.add('hidden');return;}
