...
3. description 必须详细，包含所有可见的视觉特征，以便后续语义检索。例如，人物分类必须详细描述姿势动作，包括四肢位置和身体角度，不能只写笼统动词
...
11. ref_strength 评估标准（严格判断！此字段仅评估姿势与构图的参考价值，与服装美观度完全无关，剪影判断时必须完全排除服装轮廓。目标分布：full约20%，style约40%，reimagine约40%。边界模糊时一律优先降级。每张图根据实际内容独立判断）：
   ...
   - "style"：姿势有一定变化，身体有轻微的角度或肢体调整，能传递基本氛围，但不具备独立模板价值。判断标准：姿势不单调，但也不足以作为单独模仿的对象，仅能参考其整体感觉和氛围
   - "reimagine"：姿势为纯功能性展示姿态，无独立参考价值，身体无明显倾斜、扭转或曲线，构图无设计意图。判断标准：如果只看人物轮廓剪影，这就是一个标准的"人形衣架"，姿势本身没有值得保留的视觉特征。典型包括：背对镜头展示服装、正面完全端正展示服装等
