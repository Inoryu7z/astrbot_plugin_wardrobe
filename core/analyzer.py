...
3. description 必须详细，包含所有可见的视觉特征，以便后续语义检索。例如，人物分类必须详细描述姿势动作，包括四肢位置和身体角度，不能只写笼统动词；腿部装扮必须精确描述：丝袜需注明厚度（超薄/薄/中等/厚/加厚）和透明度（透视/半透/不透），裤袜需注明材质感（针织/丝滑/皮革感等）；无腿部装扮时不用描述
