...
* database 新增 `get_ids_by_filter()` / `get_records_by_ids()` 方法
...
* 新增**视频保留策略**：自动保存的视频（DailySharing 等插件产生的）超过 7 天后自动删除文件和记录，每天凌晨 5 点清理，硬编码 7 天
* WebUI 手动生成的视频不受视频保留策略影响
* 新增**自动备份开关**（`auto_backup_enabled`）：可关闭自动备份以节省存储空间，默认开启
* 新增**无感模式**（favorite=meh）：标记为"无感"的图片超过 30 天后自动彻底删除（文件 + 数据库 + 向量索引 + 关联视频），每天凌晨 5:30 清理，硬编码 30 天
* 无感与"无标签"的区别：只有无感标签的图片才会被自动删除，无标签图片不会被自动清理
* 新增**批量标签操作**：批量模式下可一键将选中图片标记为收藏/喜欢/无感/取消标记
* WebUI 侧边栏、统计页面、右键菜单、详情弹窗均支持无感筛选和标记
* database 新增 `get_expired_auto_save_videos()` / `get_expired_meh_images()` 查询方法
