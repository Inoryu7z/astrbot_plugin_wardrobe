...
* 新增 `body_focus` 字段：记录画面聚焦的视觉重点区域，提升检索精准度
* WebUI 详情页新增"重新分析"按钮：可调用存图模型重新分析图片属性，支持输入描述辅助分析
* 新增 POST `/api/images/<id>/reanalyze` API
* WebUI 批量上传：支持多文件选择，逐张上传并显示进度
* 收藏/喜欢机制：双层标记（收藏>喜欢），取图时优先返回收藏图片；WebUI 侧边栏筛选、详情页快捷按钮
* 新增 PATCH `/api/images/<id>/favorite` API

**🔧 改进**

* 氛围池与姿势池扩展优化
* 暴露程度分级细化：5级制（保守/轻微/中等/明显/极限）
* 多个特征字段提示词优化，提升分析准确度
* 关键词搜索扩展到 7 个字段
* 检索管线补全 pose_type/body_focus 支持，修复 exposure_level 值不匹配
* 旧图重分析条件补全 allure_features 和 body_focus

### v1.9.1

**✨ 新功能**

* 新增 `allure_features` 字段：记录动作/表情/姿势带来的魅力感，提升检索维度

**🔧 改进**

* 关键词搜索扩展到 6 个字段，新增 allure_features
* 向量索引文本构建新增"魅力特征"前缀
...
* 新增三个特征字段（均为数组类型，可多选），大幅提升搜索精准度：
  - `exposure_features`：记录服装穿着特征
  - `key_features`：记录图片最突出的3-5个视觉特征
  - `prop_objects`：记录画面中的道具/物品
* WebUI 详情页全面改造：所有字段均可查看和编辑，支持标签型字段（回车添加、×删除）和文本型字段
* 新增 PUT `/api/images/<id>` API，支持编辑图片的所有属性字段
* 编辑图片属性后自动重建向量索引，确保搜索结果与最新数据一致
* 旧图自动重分析：首次启动时检测到新字段为空的旧图，逐张顺序重分析（间隔2秒），不阻塞正常使用

**🔧 改进**

* 关键词搜索现在同时匹配 description、user_tags、exposure_features、key_features、prop_objects 五个字段，搜索覆盖面大幅提升
* 向量索引文本构建纳入新字段，提升语义检索质量
* 存图分析提示词新增三个字段的输出格式和规则说明
