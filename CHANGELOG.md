...
* 热度不再全局共享：人格 A 用过的图，对人格 B 仍显示 0 热度
* 取图时 LLM 能看到候选图片在当前人格下的使用次数，内容匹配度相近时优先选低热度图
* 扩大向量检索候选池（fetch_k 倍数提升），避免相似度筛选后候选不足导致热度平衡失效
* WebUI 批量操作栏新增「清除热度」按钮，可批量清零选中图片的热度
* 升级后旧的全局 use_count 自动归零，按人格重新开始统计
* 空人格（未设置人格）取图不记热度
...
* 衣柜库新增 `ai_prompt` 字段，存储由 aiimg 插件生成该图时所用的提示词
* 仅自拍模式（`/自拍` 命令及 LLM `aiimg_generate` 工具的 selfie 路径）自动存入衣柜时会附带提示词；手动上传/`/存图` 命令保存的图该字段为空
* WebUI 详情页新增「AI提示词」字段，为空时自动隐藏（纯天然图片不显示该行）
* 数据库通过 `ALTER TABLE ADD COLUMN` 自动迁移，历史数据该字段为空
* 备份导出/导入已包含 `ai_prompt` 字段
* 字段可在 WebUI 编辑修改
