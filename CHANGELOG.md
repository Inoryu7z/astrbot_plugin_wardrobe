...
* WebUI 搜索支持向量模型：`/api/search` 优先使用向量语义检索，无结果时自动回退关键词匹配
* WebUI 批量上传：支持多选文件，批量上传图片，每张图片间隔 5 秒依次请求识图模型避免并发过载
* 搜索结果数量显示：搜索时显示匹配结果数量

**🐛 Bug 修复**

* 修复 API 返回值类型不一致导致前端 `.json()` 崩溃：`api()` 函数在非 401 错误时返回普通对象而非 Response，调用方直接调用 `.json()` 会抛出异常
